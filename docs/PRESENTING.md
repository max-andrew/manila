...
6. **Equity that pays in cash (the second rail)** — scroll to the **Equity vesting** card. Point at the pulsing oracle banner: *"AAPL, live from a Pyth feed, relayed on-chain to Arc."* *"Ada's RSUs vest as shares; on release the contract reads the live share price and pays the vested value in USDC — equity that tracks the real stock but settles in stablecoin."* Click **↻ Reset clock** (the agent re-arms the schedule on-chain, Dynamic-signed), then **Release** → a real USDC payout whose size came from the oracle. *"That's the Arc smart-contract track: an oracle read, share→USDC conversion, and settlement in one release."* **← the programmable-stablecoin moment.**
7. **The audit** — click **Open the envelope**. *"Confidential to the public, fully auditable to the employer — the compliance-correct split."*
