...
### Arc — Advanced Stablecoin Logic
A deployed `PayrollVault` contract on Arc: programmable cliff + linear USDC vesting, funded by the employer, `release()`d by the agent wallet, events feeding the audit log. Deployed at `0x2f217B2A62826F247084B207106233E5F67c60Ac`; a real vest + release ran on-chain (release tx `0x9f38e8409dbf3cd4c04f5a3bc7f2e538783a68ef0bdcbb24fea6dc4ce5bf2ba5`). Gas paid in USDC. (`contracts/`.)
...
## Notes for judges
The credential-free surface (agent, policy, maker-checker approval, live treasury balance, audit export) runs on the deployed URL. The full on-chain disbursement path runs locally against the Dynamic signing sidecar (its SDK ships a native binary that can't run on Cloudflare Workers); see the README "Run it" section. On arc-testnet the sealed token is Unlink's mock USDC (`USDCm`); the architecture is token-agnostic. AI tool usage is documented in `docs/AI_USAGE.md`, with the pre-build spec in `docs/SPEC.md` and per-sponsor DX feedback in `docs/FEEDBACK.md`.
